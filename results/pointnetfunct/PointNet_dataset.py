...
class Aneuxmodel_Dataset(Dataset):
    ...
    def __init__(self, df = pd.DataFrame(), root = "", transform = None,mesh = "area-001",cuttype = "dome",crop = False,points= 1000, limit = 700,load = True,paraout = False,train=True):
        ...
        self.df = df
        self.df_process = df[df["cuttype"] == cuttype]
        self.label = []
    ...
    def label_loader(self):
        ...
        for model in self.model_table:
            if  model in list(self.df_process["dataset"]):
                label_num = self.df_process[self.df_process["dataset"] == model]["status"]
    ...
    def training_data_load(self):
        ...
        if self.mesh == "area-001":
            ...
            IA = "aneurysms\\remeshed\\area-005"
        ...
        all_model_name = list(self.df_process["dataset"])
        lowbond = 0
        upond = len(all_model_name)
        if self.train == False:
            lowbond = self.limit 
            upond = len(all_model_name)
        else:
        ...
        for model in all_model_name[lowbond:upond]:
...
class Aneuxmodel_Dataset_ppd(Dataset):
    ...
    def training_data_load(self):
        ...
        for model in all_model_name[lowbond:upond]:
            ...
            if IA_name_cut1 in all_IA_model:
                IA_root_cut1 = os.path.join(IA_root,IA_name_cut1)
                cut1 = vtp_to_point_cloud_fps(IA_root_cut1,points = self.points)
                self.cut1_model_file.append(cut1)
            else:
                print("missing a cut1 model: " + model_name)
            ...
            if IA_name_dome in all_IA_model:
                IA_root_dome = os.path.join(IA_root,IA_name_dome)
                dome = vtp_to_point_cloud_fps(IA_root_dome,points = self.points)
                self.dome_model_file.append(dome)
            else:
                print("missing a dome model: " + model_name)
            ...
            if vessel_name in all_vessel_model:
                Vessel_model_root = os.path.join(Vessel_root,vessel_name)
                vessel = vtp_to_point_cloud_fps(Vessel_model_root,points = self.points)
                self.vessel_model_file.append(vessel)
            else:
                print("missing a vessel model: " + model_name)
            ...
            if self.crop:
                ...
                IA_root_dome = os.path.join(IA_root,IA_name_dome)
                Vessel_model_root = os.path.join(Vessel_root,vessel_name)
                Vessel_crop_cut1 = vtp_to_point_cloud_cutvessel_fps(Vessel_model_root,IA_root_cut1,points = self.points)
                Vessel_crop_dome = vtp_to_point_cloud_cutvessel_fps(Vessel_model_root,IA_root_dome,points = self.points)
                if Vessel_crop_dome == False:
                    print(Vessel_model_root)
                self.cropcut1_vessel_file.append(Vessel_crop_cut1)
                self.cropdome_vessel_file.append(Vessel_crop_dome)
...
def Aneux_Dataset_load(filepath,train = True):
