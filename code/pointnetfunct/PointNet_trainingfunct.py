...
def run_model_get(train_loader_input,vaild_loader_input,nepochs, modelnet,results_path, filename, transform = None,dataset =None):
    ...
    for epoch in range(nepochs):  # loop over the dataset multiple times
        ...
        if atst > max_atst:
            model_path = "pointnet_dome_lr0.0001_epoch_test.pth"
...
def stats_2mh(loader, net):
    ...
    with torch.no_grad():
        for data in loader:
            inputs_v, inputs_v2, labels = data
...
def run_model_2multi_head(train_loader_input,vaild_loader_input,nepochs, modelnet,results_path, filename, transform = None,dataset =None):
    os.makedirs(results_path, exist_ok = True)
    saveCkpt = results_path + filename
    statsrec = np.zeros((4,nepochs))
    modelnet = modelnet.to(device)
    ...
    for epoch in range(nepochs):  # loop over the dataset multiple times
        ...
        if atst > max_atst:
            model_path = "pointnet_2multihead_lr0.001_epoch_test.pth"
            torch.save(modelnet.state_dict(), model_path)
            max_atst = atst
...
def run_model_3multi_head(train_loader_input,vaild_loader_input,nepochs, modelnet,results_path, filename, transform = None,dataset =None):
    ...
    for epoch in range(nepochs):  # loop over the dataset multiple times
        ...
        #Save the model with highest accuracy
        if atst > max_atst:
            model_path = "pointnet_3multihead_lr0.001_epoch_test.pth"
            torch.save(modelnet.state_dict(), model_path)
            max_atst = atst
